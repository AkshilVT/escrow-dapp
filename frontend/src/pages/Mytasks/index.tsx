...
export default function Mytasks() {
  ...
  const handleUpload = (
    ...
    // };
    uploadOnChain(title, desc, amount, deposit);
    //API Call
  ...
  const uploadOnChain = async (
    ...
    try {
      if (
        ...
        await (
          await taskContract.createTask(amount, deposit, title, desc, {
            value: 100,
          })
        ).wait();
  ...
  const { walletConnectionStatus, switchNetwork, walletAddress } = useWallet();

  const getTasks = async () => {
    const taskContract = getSmartContract<EscrowContract>('ESCROWCONTRACT');

    try {
      if (walletConnectionStatus === 'connected' && taskContract) {
        switchNetwork(deployedNetworkData.chainId);
        ...
        allTasks.filter(
          (task: any) =>
            task.isAssigned === true && task.artistAddress === walletAddress
        );
        setList(allTasks);
      }
    } catch (e) {
  ...
  return (
    <>
      ...
      <div
        ...
        tabIndex={1}
        aria-hidden='true'
        className={`overflow-y-auto overflow-x-hidden fixed top-0 z-50 flex justify-center items-center w-full ${
          isModalOpen ? 'block' : 'hidden'
        }`} // Conditionally apply 'hidden' class
      >
          ...
          <div className='relative bg-white rounded-lg shadow dark:bg-gray-700'>
            ...
            <div className='flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600'>
              <button
                ...
                onClick={() =>
                  handleUpload(title, description, amount, securityDeposit)
